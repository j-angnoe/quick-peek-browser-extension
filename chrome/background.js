...
// Chrome does not have a captureTab
// implemented, which destroys the purpose
// of this extension.
    ...
    tabs: {
        ...
        executeScript(...args) {
            return new Promise((resolve, reject) => {
                chrome.tabs.executeScript(...args, resolve);
            })
        },
        insertCSS(...args) {
            return new Promise(resolve => {
                chrome.tabs.insertCSS(...args, resolve);
            });
        },
        update(...args) {
            return new Promise(resolve => {
                chrome.tabs.update(...args, resolve);
            })
        },
        query(...args) {
            return new Promise(resolve => {
...
function registerDomContentLoadedListener() { 
    var domContentLoadedListener = data => {
        ...
        // chrome has no windowId attribute available here.
        // var id = `${data.windowId}-${data.tabId}`;
        var id = data.tabId;
    
        console.log(data);
        
        console.log("DOMContentLoaded for " + id);
        console.log(loadedListenerForTab);
...
async function peek(url) {
    ...
    var lastTabUrl = null;
    // chrome has no windowId available in onDOMContentLoadedListener.
    var listenId = `${newTab.id}`;

    loadedListenerForTab[listenId] = async (data) => {
